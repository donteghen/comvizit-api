...
exports.initialize = exports.plugin = void 0;
const mongoose_1 = __importStar(require("mongoose"));
const extend = require('extend');
let identityCounterSchema;
let IdentityCounter;
// Initialize plugin by creating counter collection in database.
const initialize = function () {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            console.log('Attempting the initializtion.....');
            IdentityCounter = yield mongoose_1.default.model('IdentityCounter');
            console.log('Auto Increment Initialized successfully!');
        }
        catch (e) {
            console.log(e);
            if (e.name === 'MissingSchemaError') {
                // Create new counter schema.
                identityCounterSchema = new mongoose_1.Schema({
                    model: { type: String, require: true },
                    field: { type: String, require: true },
                    count: { type: Number, default: 0 }
                });
                // Create a unique index using the "field" and "model" fields.
                identityCounterSchema.index({ field: 1, model: 1 }, { unique: true, required: true, index: -1 });
                // Create model using new schema.
                IdentityCounter = mongoose_1.default.model('IdentityCounter', identityCounterSchema);
            }
            else {
                throw e;
            }
...
const plugin = function (schema, options) {
    ...
    if (!identityCounterSchema || !IdentityCounter)
            ...
            (function save() {
                ...
                if (ready) {
                    ...
                    if (typeof doc[settings.field] === 'number') {
                        ...
                        { count: doc[settings.field] }, function (err) {
                            if (err)
                                return next(err);
...
//# sourceMappingURL=auto-incremeny.js.map